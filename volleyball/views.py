...
def stats(request):
    kill_leaders = Player.objects.filter(kills__gt=0).order_by('-kills')[:5]
    dig_leaders = Player.objects.filter(
        digs__gt=0).order_by('-digs')[:5]
    block_leaders = Player.objects.filter(
...
def allStats(request):
    ...
    players = Player.objects.order_by('-service_aces','-kills')
