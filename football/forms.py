...
class PlayerForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            'player_first_name': forms.TextInput(attrs={'class': 'form3', 'placeholder': 'First name', 'required': 'required'}),
            'player_last_name': forms.TextInput(attrs={'class': 'form3', 'placeholder': 'Last name', 'required': 'required'}),
            'player_date_of_birth': forms.DateInput(attrs={'class': 'form3', 'type': "text", 'onfocus': "(this.type='date')", 'onblur': "(this.type='text')", 'placeholder': 'Date of Birth', 'required': 'required'}),
            'player_phone_number': forms.TextInput(attrs={'class': 'form3', 'placeholder': 'Phone number', 'required': 'required'}),
            'player_email': forms.EmailInput(attrs={'class': 'form3', 'placeholder': 'Email address', 'required': 'required'}),
            'player_image': forms.FileInput(attrs={"class": "file-input", "accept": "image/*", 'placeholder': 'Player image'}),
            'player_shirt_number': forms.TextInput(attrs={'class': 'form3', 'type': 'number', 'placeholder': 'player shirt number', 'required': 'required'}),
        }
